    ...
    def __init__(self, filas = FILAS, columnas = COLUMNAS,
        ...
        if not (isinstance(filas,int) and isinstance(columnas,int) and \
           isinstance(largo,int)):
            ...
            self.scr.onkeypress(lambda : mueva_snake("Right"), "d")   # Abajo
            self.scr.onkeypress(lambda : mueva_snake("Left"), "a")    # Izquierda
            self.scr.onkeypress(lambda : mueva_snake("Up"), "w")      # Arriba
